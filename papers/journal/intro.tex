...
%
Molecules are represented as $Q_{i}$ or $R_{i}$.
%
In the VTS, labels are a string of molecule, and red color text indicates that the molecule is active.
%
The activity of the molecules are controlled
by presence of the other molecules. 
...
\begin{example}
	%
	In figure~\ref{fig:synth-vts}, we present a partial VTS that has 3 compartments and it's complete VTS. The synthesis is performed on the basis of steady state property and the  3-connectivity of the graph. PM, EE, LE are the tags representing the node. Edge in the red colour represent the added edge and molecules in blue represent added molecules on the corresponding edge by synthesis. For the sake of clarity we have not presented the activity, pairing matrix and rest of the molecules on the nodes.
