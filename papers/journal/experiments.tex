...
%The existential condition is implemented with two different encodings. First, with a bounded model checker that uses an enumeration of paths for encoding the reachability problem: {\sattool}.
%which is based on the architecture of a
%
%The other with an SMT solver Z3 that uses the efficient encoding of finding a least fixed point to solve it: {\smttool}.
...
%
\todo{SA: Do we need a diagram}	
%The CBMC architecture consists of a C-language front-end (CFE) and a SAT Solver back-end (SAT). 
%
%CBMC front end accepts 

We encode the problem using CBMC C-language front-end (CFE) interface 
%ANSI-C programs 
with some special annotations (called assumes and assertions) to express constraints on the model and properties to be checked. 
%	
