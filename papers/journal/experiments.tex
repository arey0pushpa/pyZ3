...
%
We have developed the {\vtstool} tool to conjecture the properties of the VTS. 
%
The toolchain is divided into three different blocks. 
%
...
%
The {\ourtool} is a synthesis tool for synthesizing incomplete information in the input VTS.
%
...
The {\sattool} tool is implemented using model checking tool for C program CBMC~\cite{clarke2004tool}.
%
...
\textbf{{\smttool} (SMT based tool)}:
	We have implemented the encodings for each variants using the python interface of Z3 in a tool {\smttool}. 
%
...
\textbf{{\qbftool} (QBF based synthesis tool):}
