...
%\end{scope}
 \begin{tikzpicture}[->,>=stealth', auto , node distance=3.1cm,
	 ...
	 % 	\begin{scope}[]
	  ...
	  \path (ee) edge[bend right=32, text width=1.8cm] node [below,right=0.1] {{$R_{8}$}, $Q_{a7}$, $Q_{b7}$, $R_{7}$} (le);
...
%  \vspace{1.2cm}
  \begin{subfigure}[b]{0.49\linewidth}
	 ...
	 % \begin{scope}[xshift=3.5cm,yshift=0.0cm]
	  ...
	  %le <-> pm
	  	  \path (le) edge[bend right=45] node [above] {$Q_{b7}$, $Q_{c7}$,{\color{blue}{ \textbf{$R_{7}$}}}} (pm);
...
%	  \path (ee) edge[bend right] node [text width=1.5cm, below,rotate=50] {{R8}, Qa7, Qbc7,R7} (le);	  
	  ...
	  \path (ee) edge[bend left=30] node [below, text width=2.0cm] {{$R_{3}$,{\color{blue} { \textbf{{$Q_{a2} $}}}}, {\color{blue} {{ $ Q_{a7}$}}}, {\color{blue} {{$R_{2}$}}}}} (pm);
	   \path (pm) edge[bend left=30, text width=2.3cm] node [above,rotate=-45] {{$R_{2}$}, {$Q_{a7}$} $Q_{bc7}$, $R_{7}$, $Q_{c7}$, $Q_{a2}$} node [below,anchor=center] {\color{blue} { \textbf{    $R_{3}$}}} (ee);
