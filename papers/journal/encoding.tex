...
%
Nondeterminism can be viewed as an oracle that encodes a correct $m$-path for a specific $m$ if it exists. 
%
...
We encode stability using \texttt{NOracle} and if there is an $m$-edge between node $i$ and $j$, there has to be a $m$-path from $j$ to $i$ of length maximum w.
%
We have used bv $\unrhd$ m to extract the $m^{th}$ bit of the bitvector bv. 

\begin{align}
 \bigwedge\limits_{i,j,m} (\bigvee_{q} (\texttt{bvEdge}[i][j][q] \unrhd m) := 1) \limplies NOracle(m, j, i, w)
