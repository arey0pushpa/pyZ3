...
\begin{enumerate}
	\item Bit vector $bv\_e[i][j][q]$ represent the $q$th edge between node $i$ and $j$.
\end{enumerate}
...
\textbf{Example 1} The variable bv\_e[1][2][1] = 1001 represent first edge between node 1 and node 2 with molecule M1 and M4 present. 
...
\begin{align}
 \bigwedge\limits_{i,j,m} (\bigvee_{q} (bv\_e[i][j][q] \unrhd m) > 0) \limplies NOracle(m, j, i, w)
...
encoding of reachability, which was done using an enumeration of paths. 
%
...
\input{qbf-table}
%
In the table~\ref{tab:satqbf-graph}, you can see that SAT version was not able to scale it to the desired results.
%
This is due to the fact that directly translating a quantified formula into a quantifier-free  by introducing a vector of variables for each boolean possibility will blow up the size of the formula and may not be solvable by a SAT solver.
