...
%
\begin{align*}
  \texttt{PresentEdge} = \bigwedge\limits_{i,j,q} (\bigvee_m e_{i,j,q,m} )\limplies e_{i,j,q}
\end{align*}
The edge labels are subset of the node labels of the source and target nodes.
\begin{align*}
  \texttt{LabelEdge} = \bigwedge\limits_{i,j,q,m} e_{i,j,q,m} \limplies (n_{i,m} \land n_{j,m} )
\end{align*}
Self edges are not allowed. 
\begin{align*}
  \texttt{SelfEdge} = \bigwedge\limits_{i,q} \neg e_{i,i,q}
\end{align*}

We use \texttt{EdgeC} to collect all the edge related constraints. 
%
...
\begin{align*}
  \texttt{EdgeC} = \texttt{PresentEdge} \land \texttt{SelfEdge} \land \texttt{LabelEdge} 
\end{align*}

If a molecule is active on an edge, it should be present on the edge.
%
\begin{align*}
  \texttt{ActivityMol} = \bigwedge\limits_{i,j,q,m} b_{i,j,q,m} \limplies e_{i,j,q,m}\label{eq:f1}
\end{align*}
A molecule should be present to be active on a node.  
\begin{align*}
    \texttt{PresentMol} = \bigwedge\limits_{i,m} a_{i,m} \limplies n_{i,m}
\end{align*}

We use \texttt{ActivityC} to collect all the molecule activity related constraints. 
\texttt{ActivityC} states that active molecule are present.
\begin{align*}
  \texttt{ActivityC} = \texttt{ActivityMol} \land \texttt{PresentMol} 
\end{align*}
...
\begin{align*}
\texttt{NodeFunOn} =& \bigwedge\limits_{i,m} n_{i,m} = a_{i,m}  
\end{align*}
...
\begin{align*}
\texttt{NodeFunBf} =& \bigwedge\limits_{i,m} n_{i,m} \limplies a_{i,m} =  \nodef_m (n_{i,1},\dots,n_{i,\mu}) 
\end{align*}
...
\begin{align*}
 \texttt{EdgeFunOn} =& \bigwedge\limits_{i,j,q,m} e_{i,j,q,m} = b_{i,j,q,m}
\end{align*}
...
\begin{align*}
 \texttt{EdgeFunBf} =& \bigwedge\limits_{i,j,q,m} e_{i,j,q,m} \limplies b_{i,j,q,m} = \edgef_k(e_{i,j,q,1}, .., e_{i,j,q,\mu} )
\end{align*}
%
...
matrix. 
\begin{align*}
\texttt{EdgeFunPairing} =&	
\bigwedge\limits_{i,j,q, m}  [e_{i,j,q,m} \land   \\
...
\subsection{Encoding for $\fqbf$ search problem}
\label{enc:univ-cond}
%
...
\input{satqbf-table}
%
\todo{Fix the table; the entries of constraint is incorrect}The table~\ref{tab:satqbf-graph}, shows the search results of this encoding for the universal condition with 4 nodes and 3-connected graphs. 
%
