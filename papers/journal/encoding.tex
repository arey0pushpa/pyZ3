...
\\
\end{align*}
We define \texttt{FusionC} \change{Use of the word collect looks weird! change it.}to collect all the fusion related constraints.
% 
...
\label{enc:bmc}
\noindent Our first attempt was to encode the existential condition as a $\fbmc$ search \unsure{Should we cite it here or not?}problem~\cite{shukla2017discovering}.
%
...
To specify the stability condition, we use \improve{Nondet usage is not clear in the encoding. improve.}nondeterminism to perform enumeration.
%
...
%
%
Note that the enumeration is done only for an unwinding depth of $w$.
%
...
%
Starting with exploring $m$-${paths}_{ij}$ of length 1; a direct edge between $i$ and $j$ with molecule $m$ present, until every $m$-${paths}_{ij}$ of length $w-1$ is explored.\change{m-path looks - sign in the align}
...
%
In the $\fbmc$ encoding, reachability is encoded by enumerating all the possible paths using \improve{Do not show up in the encoding}nondeterminism.
%
...
%
This can be due to the fact that directly translating a quantified formula into a quantifier-free by introducing a vector of variables for each boolean possibility \change{Increase the formula size exponentially.. the current statement is a repeat of the forth line. Improve.}will blow up the size of the formula and may not be solvable by a solver.
%
%
