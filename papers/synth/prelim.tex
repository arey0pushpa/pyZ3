...
%
Both $\texttt{ReachBelow}(d,r')$ and $\texttt{ReachAbove}(d,r')$ defines
reachability in the modified VTS,
...
\begin{align*}
  ...
  \texttt{ReachBelow}(d,r') = &
   \bigwedge\limits_{i,j}  r^{\prime}_{i,j} \limplies [\bigvee_{q} (e_{i,j,q} \land  \neg d_{i,j,q}) \lor  (\bigvee_{i' \neq i}  r^{\prime}_{i',j} \land  \bigvee_{q} (e_{i,i',q} \land \neg d_{i,i',q}) ]\\
  \texttt{Connected}(r') = & \Land\limits_{i,j} (r^{\prime}_{i,j} \lor r^{\prime}_{j,i})
\end{align*}
We will be synthesizing for both connected and disconnected graphs.
%
We define the following macros. 
\begin{align*}
  \texttt{Connected}(k) = & \forall d.\;
          (\texttt{Drop}(d,k) \limplies \exists r'.\;\texttt{ReachBelow}(d,r')
...
quantified, the connectedness property introduces the two
