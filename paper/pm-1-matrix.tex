...
\caption{\textbf{Pairing matrix}. VTS has 3 compartments and 8 molecules. Molecules are numbered 0-7 and and intersection matrix entry describes the fusion dependency. The entry 1 represents pairing, entry $\times$ represents no dependency and rectangles colored $\times$ one represents don't cares. In the right side of the pairing matrix is the graph VTS representation. This graph is an example of version where every present molecule on the node is considered as active and activity of the molecules on the edges are described by presence of other molecules. Activity is shown by use of an overline in the grap both on the nodes and the edges. Everwhere in the graph you can see the flux is maintained, that is every molecule is coming back to it's source node in a cycle. The graph follows the fusion rules described in later sections. The resultant graph is a 3-connected graph.} \label{fig:M1}
\end{figure}
