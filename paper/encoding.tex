...
\end{enumerate}
%
The version (A) is most basic where every present molecule is active.
%
In versions (B-D,F),
most general restriction is activity of the molecule is dependent on
the other molecules presence on the node.
%
In the versions (E,F), activity of the molecules on the edges
is dictated by pairing matrix; we call this process ``pairing
inhibition".
%
In the column 2-3 of table~\ref{table1}, we list the constraints involved
in each version.
