...
\begin{df}
  ...
  \end{itemize}
\end{df}
$\pair$, $\nodef$, and $\edgef$ defines activity of a molecule on nodes and edges.
A molecule $k$ is {\em active} at node $n$ if $\nodef(k,\nlabel(n))$ is true.
A molecule $k$ is {\em active} at edges $(n,M',n')$ if $\edgef(k,M')$ is true.
