...
In this paper, we have developed a novel SAT encoding for searching a VTS that satisfy the given properties. We improved the encoding of some of the conditions for VTSs. In the earlier implementation, reachability was encoded by enumeration of paths. In our current encoding, we have recursively defined the reachability condition such that that we avoid the exponential enumeration. We have also encoded $k$-connectivity in a similar fashion and avoided a blowup in the formula size. Furthermore, our encoding supports several variations of the conditions. We have implemented the encoding using Z3~\cite{z3} python API and searched for VTS satisfying various properties upto size 14-18 nodes as compare earlier tool that could only search graph upto size 8 nodes. Our tool is useful to cell biologists since VTSs of real cells have approximately ten compartments.  \\
