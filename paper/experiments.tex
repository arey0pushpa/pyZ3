...
We have
implemented~\footnote{\url{https://github.com/arey0pushpa/pyZ3}} the
encodings for each variants using the python interface of $\zthree$ in
a tool (MAA).
%
...
in last column of the table~\ref{tab:grph}, we also present the minimum connectedness needed for
...
number of molecules to be twice as number of the nodes for N $>$ 2 and M
= 2N + 1 for N = 2. For each version we fix total number of parallel
edges to maximum two. In the table we have shown comparison for
specific connectivity, for example Variant A is checked against any
graph with connectivity 2, Variant B with connectivity 3 similarly for
